...
namespace UnityGameFramework.Runtime
{
    ...
    /// 默认全局配置辅助器。
    /// </summary>
    public class PreloadConfigHelper : ConfigHelperBase
    {
        ...
        public override bool ParseData(IConfigManager configManager, string configString, object userData)
        {
            try
            {
                ...
                while ((configLineString = configString.ReadLine(ref position)) != null)
                {
                    ...
                    {
                        Log.Error("Can not parse config line string '{0}' which column count is invalid.", configLineString);
                        return false;
                    }

                    string configName = splitedLine[0];
                    string configValue = splitedLine[1];
                    if (!configManager.AddConfig(configName, configValue))
                    {
                        Log.Error("Can not add config with config name '{0}' which may be invalid or duplicate.", configName);
                        return false;
                    }
                }
        ...
        public override bool ParseData(IConfigManager configManager, byte[] configBytes, int startIndex, int length, object userData)
        {
            ...
            {
                Log.Error("Can not parse config bytes with exception '{0}'.", exception);
                return false;
            }
        }
