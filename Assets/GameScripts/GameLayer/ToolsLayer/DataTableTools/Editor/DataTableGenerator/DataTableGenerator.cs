...
namespace GameDevScript.EditorTools
{
    ...
    {
        private string CSharpCodePath = null;
        private string TxtFileFullPath = null;
        private const string CSharpCodeTemplateFileName = "Assets/GameScripts/GameLayer/ToolsLayer/DataTableTools/DataTableCodeTemplate.txt";
        private readonly Regex EndWithNumberRegex = new Regex(@"\d+$");
        private readonly Regex NameRegex = new Regex(@"^[A-Z][A-Za-z0-9_]*$");

        public DataTableProcessor CreateDataTableProcessor(string dataTableName)
        {
            ...
            throw new GameFrameworkException($"CreateDataTableProcessor dataTableName={dataTableName}");
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="txtFileFullPath"></param>
        /// <param name="encoding">null时设置为Encoding.UTF8</param>
        /// <returns></returns>
        public DataTableProcessor CreateDataTableProcessor(string txtFileFullPath, string csFolderPath, Encoding encoding = null)
        {
            TxtFileFullPath = txtFileFullPath;
            CSharpCodePath = csFolderPath;
            if (string.IsNullOrEmpty(txtFileFullPath))
            {
                throw new GameFrameworkException("读取的txt路径为空");
            }
            if (string.IsNullOrEmpty(csFolderPath))
            {
                throw new GameFrameworkException($"存放的脚本路径为空");
            }

            if (encoding == null)
                encoding = Encoding.UTF8; //Encoding.GetEncoding("GB2312")
            return new DataTableProcessor(Utility.Path.GetRegularPath(txtFileFullPath), encoding, 1, 2, null, 3, 4, 1);
        }

        public bool CheckRawData(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                {
                    Debug.LogWarning(Utility.Text.Format("Check raw data failure. DataTableName='{0}' TName='{1}'", dataTableName, name));
                    return false;
                }
            }
        ...
        public void GenerateCodeFile(DataTableProcessor dataTableProcessor, string dataTableName, string useNamespace)
        {
            ...
            if (!dataTableProcessor.GenerateCodeFile(csharpCodeFileName, Encoding.UTF8, new string[] { dataTableName, useNamespace }) && File.Exists(csharpCodeFileName))
            {
                File.Delete(csharpCodeFileName);
            }
        }

        private void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            string[] userDataArray = userData as string[];
            string dataTableName = userDataArray[0];
            string useNamespace = userDataArray[1];

            codeContent.Replace("__DATA_TABLE_CREATE_TIME__", DateTime.UtcNow.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"));
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", useNamespace);
        ...
        private string GenerateDataTableProperties(DataTableProcessor dataTableProcessor)
        {
        ...
        private string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder
                .AppendLine("        public override bool ParseDataRow(string dataRowString, object userData)")
                .AppendLine("        {")
                .AppendLine("            string[] columnStrings = dataRowString.Split(DataTableRuntimeParseTools.DataSplitSeparators);")
                .AppendLine("            for (int i = 0; i < columnStrings.Length; i++)")
                .AppendLine("            {")
                .AppendLine("                columnStrings[i] = columnStrings[i].Trim(DataTableRuntimeParseTools.DataTrimSeparators);")
                .AppendLine("            }")
                .AppendLine()
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                {
                    string typeName = dataTableProcessor.GetTypeString(i);
                    typeName = typeName.Replace("[]", "Array");
                    typeName = typeName.Substring(0, 1).ToUpper() + typeName.Substring(1);
                    stringBuilder.AppendFormat("            {0} = DataTableRuntimeParseTools.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i), typeName).AppendLine();
                }
            }
        ...
        private string GenerateDataTablePropertyArray(DataTableProcessor dataTableProcessor)
        {
