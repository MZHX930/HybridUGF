...
Shader "Custom/GuideHighlight2"
{
    Properties
    {
        ...
        // 自定义参数
        _Center("Center",vector) = (0,0,0,0)  // 矩形中心坐标（屏幕像素坐标）
        _HalfWidth("HalfWidth",Range(0,2000))=100    //半宽度 （像素）
        _HalfHeight("HalfHeight",Range(0,2000))=100 //半高度（像素）
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // float _Slider;   // 控制过渡效果的阈值
            float _HalfWidth;//半宽度
            float _HalfHeight;//半高度
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // 计算当前片元到圆心的距离
                float inX = step(_HalfWidth, distance(IN.worldPosition.x , _Center.x));
                float inY = step(_HalfHeight, distance(IN.worldPosition.y , _Center.y));
                color.a *= (1- (1 - inX) * (1 - inY)); // 圆外不透明，圆内透明
