...
{
    public class TMPro_FontAssetCreatorWindow_Auto : EditorWindow
    {
        public static TMPro_FontAssetCreatorWindow_Auto ShowFontAtlasCreatorWindow(TMP_FontAsset fontAsset, TextAsset charactersAsset)
        {
            var window = GetWindow<TMPro_FontAssetCreatorWindow_Auto>();

            window.titleContent = new GUIContent("Font Asset Creator Auto");
            window.Focus();
            ...
            window.m_CharactersFromFile = charactersAsset;
            window.m_CharacterSetSelectionMode = 8; //使用字库裁剪

            window.m_ToGenerateFontAtlas = false;
            window.m_ToSaveAtlas = false;

            return window;
        }

        private bool m_ToGenerateFontAtlas = false;
        private bool m_ToSaveAtlas = false;
        public void ToGenerateFontAtlas()
        {
            m_ToGenerateFontAtlas = true;
        }
        public bool IsDoneGenerateFontAtlas()
        {
            return m_IsProcessing == false && m_ToGenerateFontAtlas == false && m_IsGlyphPackingDone == false && m_IsGlyphRenderingDone == false && m_IsRenderingDone == false;
        }

        public void ToSaveAtlas()
        {
            m_ToSaveAtlas = true;
        }

        public bool IsDoneSaveAtlas()
        {
            return m_ToSaveAtlas == false && m_FontAtlasTexture == null;
        }
        ...
        void DrawControls()
        {
            ...
            EditorGUI.BeginDisabledGroup(true);
            {
                ...
                Debug.Log($"m_CharacterSetSelectionMode: {m_CharacterSetSelectionMode}");
                switch (m_CharacterSetSelectionMode)
                {
                    ...
                    case 8: // Character List from File
                        ...
                        Debug.Log($"m_CharactersFromFile: {AssetDatabase.GetAssetPath(m_CharactersFromFile)}");
                        if (EditorGUI.EndChangeCheck())
                        {
                            m_IsFontAtlasInvalid = true;
                        }
            ...
            if (m_ToGenerateFontAtlas && GUI.enabled) //GUILayout.Button("Generate Font Atlas")
            {
            ...
            if (m_ToSaveAtlas && GUI.enabled)//GUILayout.Button("Save")
            {
