...
namespace UnityGameFramework.Editor.ResourceTools
{
    ...
    public sealed class ResourceCollection
    {
        ...
        private static readonly Regex ResourceNameRegex = new Regex(@"^([A-Za-z0-9\._-]+/)*[A-Za-z0-9\s\._-]+$");
            ...
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (!AssignAsset(guid, name, variant))
                    {
                        ...
                        Debug.LogWarning(Utility.Text.Format("Can not assign asset '{0}' to resource '{1}'.", AssetDatabase.GUIDToAssetPath(guid), GetResourceFullName(name, variant)));
                        continue;
                    }
                }
        ...
        private bool IsValidResourceName(string name, string variant)
        {
            ...
            {
                Debug.LogWarning($"{name} 不符合正则匹配规则A");
                return false;
            }

            if (variant != null && !ResourceVariantRegex.IsMatch(variant))
            {
                Debug.LogWarning($"{name} 不符合正则匹配规则B");
                return false;
            }
